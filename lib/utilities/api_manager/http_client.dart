...
import 'package:hello_dish_app/screens/FavourateScreen/models/fav_res_model.dart';
...
import 'package:hello_dish_app/screens/search/model/orderModel.dart';
...
class HTTPClient {
  ...
  static Future<SearchOrderModel> setFilterItem(
      String api, dynamic body) async {
    var token = SharedPref.shared.pref!.getString(PrefKeys.token)! ?? "";
    ...
    Get.log(res.body);
    return searchOrderModelFromJson(res.body);
  }

  static Future<FavResturantModel> getFavRest(String api) async {
    final url = Uri.parse(api);
    var token = SharedPref.shared.pref!.getString(PrefKeys.token)! ?? "";
    ...
    print(res.body.toString());
    isList = false;
    return favResturantModelFromJson(res.body);
  }
}
